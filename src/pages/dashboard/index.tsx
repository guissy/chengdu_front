...
import { getDashboardOptions } from '@/api/@tanstack/react-query.gen.ts';
import RecentAuditLogs from '@/pages/audit/recent-audit-logs.tsx';

const Dashboard = () => {
  // 这些数据在实际应用中应该从API获取
  const statsRaw = [
    { id: 1, title: '商圈总数', value: 0, key: 'cbdCount', icon: <FiMap className="h-8 w-8" />, color: 'bg-blue-500' },
    { id: 2, title: '物业分区', value: 0, key: 'partCount', icon: <FiGrid className="h-8 w-8" />, color: 'bg-green-500' },
    { id: 3, title: '铺位总数', value: 0, key: 'positionCount', icon: <FiMapPin className="h-8 w-8" />, color: 'bg-yellow-500' },
    { id: 4, title: '店铺总数', value: 0, key: 'shopCount', icon: <FiShoppingBag className="h-8 w-8" />, color: 'bg-red-500' },
